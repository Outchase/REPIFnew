...
if (isset($_POST['showScriptTable'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        for ($i=0; $i<mysqli_num_rows($result); $i++) {     //loop through the result and adds into declared array ($a)
            ...
            $output.="<tr><td>".$row['idScriptName']."</td><td>".$row['dtPath']."</td><td>".$row['dtDescription']."</td><td><button onclick=editScript('".$row['idScriptName']."','".$row['dtPath']."')>Edit</button></td><td><button onclick=deleteScript('".$row['idScriptName']."','".$row['dtPath']."')>Delete</button></td></tr>";
        }
        $output.="</tbody></table>";
    } else{

...
if (isset($_POST['showSwitchTable'])){
    ...
    if ($result->num_rows > 0) {                //Return the number of rows in a result set
        $output="<h3>Switch lists:</h3><table><thead><tr><th>Function Code</th><th>Description</th><th>Sequence Number</th><th>Delay</th></tr></thead><tbody>";
        for ($i=0; $i<mysqli_num_rows($result); $i++){
            $row = mysqli_fetch_assoc($result);
            $output.="<tr><td>".$row['idShouldFunctionsCode']."</td><td>".$row['dtDescription']."</td><td>".$row['dtSequenceNr']."</td><td>".$row['dtDelay']."</td><td><button onclick=showSwitchEdit('".$row['idShouldFunctionsCode']."','".$row['fiHostname']."','".$row['fiEventCode']."',".$row['fiPinNr'].",".$row['fiGroupNr'].")>Edit switch</button></td><td><button onclick=deleteSwitch('".$row['idShouldFunctionsCode']."','".$row['fiHostname']."','".$row['fiEventCode']."',".$row['fiPinNr'].",".$row['fiGroupNr'].")>Delete switch</button></td></tr>";
        }
        $output.= "</tbody></table><div id='switchEditContent'></div>";

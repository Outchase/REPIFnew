...
if (isset($_POST['showSwitchTable'])){
    ...
    if ($result->num_rows > 0) {                //Return the number of rows in a result set
        ...
        for ($i=0; $i<mysqli_num_rows($result); $i++){
            ...
            $output.="<tr><td>".$row['idShouldFunctionsCode']."</td><td>".$row['dtDescription']."</td><td>".$row['dtSequenceNr']."</td><td>".$row['dtDelay']."</td><td><button>Edit switch</button></td><td><button>Delete switch</button></td></tr>";
        }
        $output.= "</tbody></table>";

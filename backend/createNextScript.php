...
if (isset($_POST['executeNextScript'])) {
    ...
    $sql = "SELECT tblExecuteTheSwitch.idShouldFunctionsCode, tblPin.dtDescription, tblExecuteTheSwitch.fiEventCode, tblGroup.dtGroupName, tblUse.fiScriptName
    FROM tblExecuteTheSwitch 
        INNER JOIN tblPin ON tblPin.idPinNr=tblExecuteTheSwitch.fiPinNr AND tblExecuteTheSwitch.fiHostname=tblPin.fiHostname
    ...
    WHERE tblExecuteTheSwitch.fiHostname = '$hostname'";         //query that selects all the smarboxes of the giving user from Table tblConfigure
    ...
    if ($result->num_rows > 0) {                //Return the number of rows in a result set
        for ($i = 0; $i < mysqli_num_rows($result); $i++) {                     //the fetched values from database
            ...
            if ($row['fiScriptName'] === NULL) {
                $output .= $row['idShouldFunctionsCode'] . ",". $row['dtGroupName'];
            } else {

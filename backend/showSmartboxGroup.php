...
if (isset($_POST['smGr'])){
    ...
    $sql= "SELECT tblAffect.fiGroupNr, tblGroup.dtGroupName, tblGroup.dtDescription FROM tblAffect INNER JOIN tblGroup ON tblAffect.fiGroupNr=tblGroup.idGroupNr WHERE fiHostname = '$hostname' GROUP BY tblAffect.fiGroupNr";   //query that selects group from selected Smartbox and group them together
    ...
    if ($result->num_rows > 0) {                //Return the number of rows in a result set
        for ($i=0; $i<mysqli_num_rows($result); $i++) {
            ...
            $output.= "<div> <h4>Group ". $row['fiGroupNr'] . ": ". $row['dtGroupName'] ." (". $row['dtDescription'] .")</h4>";
